...
# get germline reference
dataP1 <- read.table(file = "P1_I64_S1_collapsed.tsv", sep = '\t', header = TRUE)
...
for (g in genes) {
  ...
  for (a in alleles) {
    ...
    novel <- findNovelAlleles(sub, germline[a] ,v_call="v_call", j_call="j_call",
                              ...
                              germline_min=1, seq="sequence_alignment", min_seqs = 1, pos_range = 1:318, alpha = 0.01, j_max = 0.3, mut_range = 1:9 )
